...
import { deleteProduct, IDeleteProductAction, SelectAction, selectProduct } from '../actions';
...
class ProductSectionRaw extends React.Component<IProductSectionProps, IProductSectionState> {
			...
			<div>
				...
				{this.state.isEditing ? (
					<EditProduct
						...
						initialValues={this.getActiveProductInfo()}
					/>
				) : (
					<AddProduct />
				)}
	...
	getActiveProductInfo(): IProductFormState {
		const err = {
			name: '',
			price: 0
		};
...
const mapDispatchToProps = (dispatch: Dispatch<SelectAction | IDeleteProductAction>, ownProps: any) => {
	return {
