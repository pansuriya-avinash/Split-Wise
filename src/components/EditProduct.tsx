...
import { editProduct, IEditProductAction } from '../actions';
...
class EditProductRaw extends React.Component<IEditProductProps, IEditProductState> {
	...
	render() {
		const ownerInfo = (
			<p>
				New product owner will be: <b>{this.props.selectedPersonName}</b>
			</p>
		);
	...
	switchCheckboxState(event: React.FormEvent<HTMLInputElement>) {
...
const mapDispatchToProps = (dispatch: Dispatch<IEditProductAction>, ownProps: any) => {
	return {
		dispatchEditProduct: (id: number, name: string, price: number, changeOwner: boolean) =>
			dispatch(editProduct(id, name, price, changeOwner))
	};
};
